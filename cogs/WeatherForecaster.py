...
class WeatherForecaster(commands.Cog):
    ...
    async def weather(self, ctx, *, city: str):
        unit_of_measurement = "metric"  # standard = kelvin, metric = celsius, imperial = Fahrenheit
        url = f"{self.baseUrl}q={city}&units={unit_of_measurement}&appid={self.apikey}"
        ...
        embed.add_field(name="Feels like", value=f"{feels_like}", inline=False)
        embed.add_field(name="Wind speed", value=f"{wind_speed}", inline=False)
